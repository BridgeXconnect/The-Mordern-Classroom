...
export type InfographicTemplate = "vocabulary-grid" | "grammar-card" | "timeline";
...
function vocabularyGridHtml(title: string, subtitle: string | undefined, words: { word: string; definition: string; example?: string }[]): string {
  const cards = words.slice(0, 9).map((w) => `
    <div style="background:#EEF2FF;border:1px solid #C7D2FE;border-radius:8px;padding:16px;">
      <div style="font-size:18px;font-weight:700;color:#4F46E5;margin-bottom:4px;">${w.word}</div>
      <div style="font-size:13px;color:#374151;margin-bottom:6px;">${w.definition}</div>
      ${w.example ? `<div style="font-size:12px;color:#6B7280;font-style:italic;">"${w.example}"</div>` : ""}
    </div>
  `).join("");
...
function grammarCardHtml(title: string, rule: string, explanation: string, examples: string[]): string {
  ...
  return `<!DOCTYPE html><html><head><style>
    ...
    <div style="background:#EEF2FF;border-radius:10px;padding:20px 24px;">
      ...
      <div style="font-size:18px;font-weight:600;color:#1E1B4B;">${rule}</div>
    </div>
    <div style="font-size:14px;color:#374151;line-height:1.6;">${explanation}</div>
    <div>
  ...
  const items = events.slice(0, 8).map((e, i) => `
    <div style="display:flex;gap:20px;align-items:flex-start;">
      ...
      <div style="padding-bottom:20px;">
        <div style="font-size:12px;font-weight:700;color:#4F46E5;margin-bottom:2px;">${e.date}</div>
        <div style="font-size:14px;color:#374151;">${e.event}</div>
      </div>
    </div>
  `).join("");
